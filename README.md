...
1. [WenetSpeech](./docs/wenetspeech.md) (10000小时，普通话) 的预训练模型列表：

|   使用模型    | 是否为流式 | 预处理方式 | test_net | test_meeting | aishell_test |   下载地址   |
|:---------:|:-----:|:-----:|:--------:|:------------:|:------------:|:--------:|
| Conformer | True  | fbank |          |              |              | 加入知识星球获取 |


2. [AIShell](https://openslr.magicdatatech.com/resources/33) (179小时，普通话) 的预训练模型列表：

|    使用模型     | 是否为流式 | 预处理方式 | 测试集字错率 |   下载地址   |
|:-----------:|:-----:|:-----:|:------:|:--------:|
|  Conformer  | True  | fbank |        | 加入知识星球获取 |
| Deepspeech2 | True  | fbank |        | 加入知识星球获取 |


3. [Librispeech](https://openslr.magicdatatech.com/resources/12) (960小时，普通话) 的预训练模型列表：

|        使用模型         | 是否为流式 | 预处理方式 | 测试集词错率 |   下载地址   |
|:-------------------:|:-----:|:-----:|:------:|:--------:|
|      Conformer      | True  | fbank |        | 加入知识星球获取 | 
|     Deepspeech2     | True  | fbank |        | 加入知识星球获取 | 
...
|   使用模型    | 是否为流式 | 预处理方式 |       数据集       | 语言  | 测试集词错率 |   下载地址   |
|:---------:|:-----:|:-----:|:---------------:|:---:|:------:|:--------:|
| Conformer | True  | fbank |      粤语数据集      | 粤语  |        | 加入知识星球获取 |
| Conformer | True  | fbank | 更大数据集（13000+小时） | 普通话 |        | 加入知识星球获取 |


**说明：** 
1. 这里字错率或者词错率是使用`eval.py`程序并使用`attention_rescoring`解码器计算得到的。
2. 由于算力不足，这里只提供了流式模型，但全部模型都支持流式和非流式的，在配置文件中`streaming`参数设置。
